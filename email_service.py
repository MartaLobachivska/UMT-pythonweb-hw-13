from fastapi_mail import ConnectionConfig, FastMail, MessageSchema, MessageType

from config import settings
from security import create_email_token, create_password_reset_token


mail_config = ConnectionConfig(
    MAIL_USERNAME=settings.MAIL_USERNAME,
    MAIL_PASSWORD=settings.MAIL_PASSWORD,
    MAIL_FROM=settings.MAIL_FROM,
    MAIL_PORT=settings.MAIL_PORT,
    MAIL_SERVER=settings.MAIL_SERVER,
    MAIL_STARTTLS=settings.MAIL_STARTTLS,
    MAIL_SSL_TLS=settings.MAIL_SSL_TLS,
    USE_CREDENTIALS=True,
    VALIDATE_CERTS=True,
)


async def send_verification_email(email: str, username: str) -> None:
    """Send an account-email verification link without exposing a password."""
    token = create_email_token({"sub": email})
    verify_url = f"{settings.VERIFY_EMAIL_BASE_URL}?token={token}"


    print("=" * 70)
    print(f"ПОСИЛАННЯ ДЛЯ ПІДТВЕРДЖЕННЯ EMAIL ({email}):")
    print(verify_url)
    print("=" * 70)

    html = (
        f"<h3>Вітаємо, {username}!</h3>"
        f"<p>Підтвердіть email: <a href='{verify_url}'>Підтвердити адресу</a></p>"
    )
    message = MessageSchema(
        subject="Підтвердження email",
        recipients=[email],
        body=html,
        subtype=MessageType.html,
    )
    try:
        await FastMail(mail_config).send_message(message)
    except Exception as e:
        print(f"Лист не надіслано (це нормально без реального SMTP): {e}")


async def send_password_reset_email(email: str, username: str) -> None:
    """Send a short-lived password-reset link to the account email address."""
    token = create_password_reset_token({"sub": email})
    reset_url = f"{settings.RESET_PASSWORD_BASE_URL}?token={token}"
    html = (
        f"<h3>Вітаємо, {username}!</h3>"
        f"<p>Для скидання пароля відкрийте: <a href='{reset_url}'>Скинути пароль</a></p>"
        "<p>Якщо ви не надсилали запит, просто проігноруйте цей лист.</p>"
    )
    message = MessageSchema(
        subject="Скидання пароля",
        recipients=[email],
        body=html,
        subtype=MessageType.html,
    )
    try:
        await FastMail(mail_config).send_message(message)
    except Exception as error:
        print(f"Лист для скидання пароля не надіслано: {error}")
